...
require_once dirname(__FILE__)."/Exception/XmlrpcException.php";
class WordpressClient
{
    ...
    private function _requestWithCurl()
    {
        ...
        if (curl_errno($ch))
        {
            ...
            $this->_logError();
            curl_close($ch);
            throw new NetworkException($message, $code);
        }
    ...
    private function _requestWithFile()
    {
        ...
        try
        {
            ...
            if ($file === false)
            {
                ...
                $this->_logError();
                throw new NetworkException($error, 127);
            }
        }
        catch (\Exception $ex)
        {
            ...
            $this->_logError();
            throw new NetworkException($ex->getMessage(), $ex->getCode());
        }
        return $file;
    }
